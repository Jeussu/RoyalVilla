...
namespace RoyalVilla_API.Controllers
{
    ...
    public class VillaAmentiesController : ControllerBase
    {
        ...
        {
            var vilas = await _db.VillaAmenities.ToListAsync();
            var dtoResponseVillaAmen = _mapper.Map<IEnumerable<VillaAmentiesDTO>>(vilas);
            var response = ApiResponse<IEnumerable<VillaAmentiesDTO>>.Ok(dtoResponseVillaAmen, "Villa Amenities retrieved successfully");
            return Ok(response);
        }
        ...
        public async Task<ActionResult<ApiResponse<VillaAmentiesDTO>>> GetVillaAmenitiesById(int id)
        {
            try
            {
                ...
                return Ok(ApiResponse<VillaAmentiesDTO>.Ok(_mapper.Map<VillaAmentiesDTO>(villaAmenities), "Record retrieved successfully"));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<ApiResponse<VillaAmentiesDTO>>> CreateVillaAmenities(VillaAmentiesCreateDTO villaAmentiesDTO)
        {
            try
            {
                ...
                var villaExists = await _db.Villas.FirstOrDefaultAsync(u => u.Id == villaAmentiesDTO.VillaId);

                if (villaExists == null)
                {
                    return Conflict(ApiResponse<object>.Conflict($"A villa with the ID '{villaAmentiesDTO.VillaId}' does not exists."));
                }
